...
struct event {
	const char *name;
	int (*fd)(struct event *);
	struct event *(*wait)(struct event *);
...
static struct event *_subproc_wait(struct event *e) {
	char buf;
...
struct event *event_wait(struct event *e) {
	assert(e->wait);
	return e->wait(e);
}
...
static struct event *_composite_wait(struct event *e) {
